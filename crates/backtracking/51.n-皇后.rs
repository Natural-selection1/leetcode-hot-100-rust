...
impl crate::Solution {
    pub fn solve_n_queens(size: i32) -> Vec2D<String> {
        ...
        let mut result = vec![];
        let mut chessboard = vec![vec![EMPTY; size]; size];
        dfs(0, size, &mut chessboard, &mut result);
        result
    }
}
...
fn is_valid(row: usize, col: usize, chessboard: &Vec2D<char>) -> bool {
    // 同列上方
    !(0..row).any(|row| chessboard[row][col] == QUEEN)
        // 对角左上
        && !(1..=row.min(col)).any(|step| chessboard[row - step][col - step] == QUEEN)
        // 对角右上
        && !(1..=row.min(chessboard.len() - 1 - col))
            .any(|step| chessboard[row - step][col + step] == QUEEN)
}
// @lc code=end
